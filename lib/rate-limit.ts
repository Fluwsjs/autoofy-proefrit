...
/**
 * Login rate limiter - limits login attempts per IP
 * More strict than other rate limiters due to security concerns
 */
export const loginRateLimit = rateLimitByIp({
  maxRequests: 5, // 5 login attempts
  windowMs: 15 * 60 * 1000, // 15 minutes
})

/**
 * Check if a specific email is being rate limited for login
 */
export function checkLoginRateLimit(email: string): { limited: boolean; remainingTime?: number } {
  ...
  if (entry && entry.resetAt > now && entry.count >= 5) {
    return {
      limited: true,
      remainingTime: Math.ceil((entry.resetAt - now) / 1000)
    }
  }
...
/**
 * Record a failed login attempt for a specific email
 */
export function recordFailedLogin(email: string): { attemptCount: number; lockedOut: boolean; lockoutTime?: number } {
  const key = `login-attempt:${email.toLowerCase()}`
  const now = Date.now()
  ...
  const lockedOut = entry.count >= maxAttempts
  
  return {
    attemptCount: entry.count,
    lockedOut,
    lockoutTime: lockedOut ? Math.ceil((entry.resetAt - now) / 1000) : undefined
  }
}

/**
 * Clear login attempts for a specific email (on successful login)
 */
